		...
		<div className="flex flex-row mx-auto max-w-screen-xl gap-8 pt-8">
			...
			<div className="flex flex-col w-full  gap-8 pt-2">
				...
				{singleSpace.videos.map((video, index) => (
					<div key={index}
						 className="flex gap-8 border border-gray-300 rounded-md p-8 cursor-pointer hover:scale-[1.01] hover:ease-in-out transition-transform duration-300 bg-gray-200">
						<img src={video.thumbnail} className="w-96 h-56 object-cover rounded-xl"/>
						<div>
							<div>

								{video.tags.map((tag, index) => (
									<span key={index} className="text-sm text-primary-800">{tag.name}</span>
								))}
							...
							<div className="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{video.title}</div>
